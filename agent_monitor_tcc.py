"""
Agente de monitoramento para TCC:
...
- Pseudonimiza hostman via SALT (LGPD)
- Parâmetro por CLI: --interval, --duration, --outdir, --salt

Observações:
- cpu_percent(inteval=1.0) bloqueia por ~1s e estabiliza a leitura
...
def sample_cpu_mem() -> Dict[str, float]:
    ...
    ram_pct = vm.percent
    mem_available_pct = (vm.available * 100 / vm.total) if vm.total else 0.0
...
def open_write(path: str, fieldnames):
...
def main():
    ...
    parser.add_argument('--salt', type=str, default=os.getenv('AGENT_SAL', 'CHANGE-ME'), help='SALT para pseudonimização')
    args = parser.parse_args()
    
    ensure_dir(args.outdir)
    
    os_name, os_release = platform.system(), platform.release()
    hostname = platform.mode() or 'unknown-host'
    ...
    f, writer = open_write(csv_path, fields)
